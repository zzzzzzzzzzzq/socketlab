    ...
    while (1)
    {
        ...
        while ((readret = recv(client_sock, buf, BUF_SIZE, 0)) >= 1)
        {
            ...
            else if (strcmp(request->http_method, "GET") != 0 && strcmp(request->http_method, "POST") != 0 && strcmp(request->http_method, "HEAD") != 0)
            {
                ...
                log_request(client_ip, request_line, 501, strlen(buf));
            ...
            else if (strcmp(request->http_version, "HTTP/1.1") != 0)
            {
                ...
                strcat(request_line, request->http_uri);
                strcat(request_line, request->http_version);
                log_request(client_ip, request_line, 505, strlen(buf));
                free(request->headers);
                free(request);
            ...
            else
            {
                ...
                {
                    log_request(client_ip, request_line, 404, strlen(buf));
                    strcpy(buf, "HTTP/1.1 404 Not Found\r\n\r\n");
                }
                log_request(client_ip, request_line, 200, strlen(buf));
                free(request->headers);
                free(request);
            }
